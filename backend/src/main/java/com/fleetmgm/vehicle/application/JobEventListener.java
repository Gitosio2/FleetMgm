...
public class JobEventListener {
    ...
    public void onJobCompleted(JobCompletedEvent event) {
        ...
        // already returned 200 OK, so a failure here (e.g. the vehicle was deleted between job start
        // and completion) can't roll anything back — it must be logged with enough context to debug
        // later, never left silent and never rethrown (same contract as InvoiceJobCompletionListener,
        // which this listener runs alongside for the same event).
        try {
            ...
            log.error("Failed to record usage update for completed job {} on vehicle {}",
                    event.jobId(), event.vehicleId(), e);
        }
    }
}

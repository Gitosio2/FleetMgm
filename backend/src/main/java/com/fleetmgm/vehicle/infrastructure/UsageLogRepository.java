...
public interface UsageLogRepository extends JpaRepository<UsageLog, UUID> {
    ...
    Optional<Long> findLatestValueBeforeDate(@Param("vehicleId") UUID vehicleId, @Param("from") LocalDate from);
     ...
     * {@link #findLatestValueBeforeDate} for why the boundary is anchored via {@code AT TIME ZONE
     * 'UTC'} instead of a bare date cast.
     */
    @Query(value = "SELECT ul.value FROM usage_logs ul WHERE ul.vehicle_id = :vehicleId "
            + "AND ul.recorded_at < (CAST(:to AS date) + INTERVAL '1 day') AT TIME ZONE 'UTC' "
            + "ORDER BY ul.recorded_at DESC LIMIT 1", nativeQuery = true)
    Optional<Long> findLatestValueUpToDate(@Param("vehicleId") UUID vehicleId, @Param("to") LocalDate to);

    /**
     * Earliest ever reading for a vehicle — used as the baseline when {@code from} is unset
     * (unbounded start) in {@code ProfitabilityService.computeUsageInRange}.
     */
    @Query(value = "SELECT ul.value FROM usage_logs ul WHERE ul.vehicle_id = :vehicleId "
            + "ORDER BY ul.recorded_at ASC LIMIT 1", nativeQuery = true)
    Optional<Long> findEarliestValue(@Param("vehicleId") UUID vehicleId);
}

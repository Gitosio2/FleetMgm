...
Migration sequence: V1 users → V2 clients → V3 vehicles → V4 workers (+ driver_vehicle_assignments) → V5 jobs (+ usage_logs) → V6 maintenance/workshop → V7 invoices (+ supplier_invoices) → V8 gps/audit → V9 vehicle license plate unique index → V10 worker national ID unique index → V11 maintenance_records `deleted_at` soft-delete column (Hito 24) → V12 maintenance_records `category` (preventive/corrective, standalone addendum) → V13 workshop_schedules `deleted_at` soft-delete column (Hito 26) → V14 workshop_schedules/maintenance_records start/end time-of-day columns (Hito 28) → V15 jobs `price` column (Hito 31) → V16 invoice number sequence (Hito 31) → V17 drop dead `MaintenanceRecord.invoice`/`InvoiceLineItem.linkedMaintenance` links (unused fields cleanup, standalone addendum) → V19 suppliers master entity + supplier_invoices.supplier_id FK (replaces supplier_name) (Hito 36) → V20 seed demo data (Hito 46) → V21 unique active assignment per vehicle (DB-level backstop, symmetric to the per-driver index from V4) → V22 backfill `due_date` for seeded invoices (forward fix for a `V20` seeding gap, standalone addendum) → V23 `jobs.version` optimistic-locking column (prevents duplicate `JobCompletedEvent` publication from a concurrent start/complete/cancel race, standalone addendum) → V24 replace `idx_gps_vehicle_id` with composite `idx_gps_vehicle_recorded` on `gps_positions(vehicle_id, recorded_at DESC)` (backs the per-vehicle latest-position lookup; the vehicle_id-only index became redundant write overhead once the composite covers the same prefix, standalone addendum). V1–V8 shipped the full schema (including tables for features not yet implemented at the application layer) in the initial scaffold commit. There is no V18 — it was reserved for seed data before the suppliers work (V19) landed first, and seeding ended up shipping later as V20; the gap is intentional, not a missing file. V20 and V22 live under `db/seed/` (not `db/migration/`) and are only added to Flyway's migration locations under the `demo` Spring profile — a plain `prod` deploy never applies them. Note: `maintenance_records` shipped in V6 without `deleted_at`; V11 adds it so the entity can adopt the standard `@SQLRestriction("deleted_at IS NULL")` soft-delete pattern like every other entity. Same rationale applies to `workshop_schedules` and V13 (Hito 26).
